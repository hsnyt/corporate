...
const ParallaxText: FC<ParallaxTextProps> = ({ scrollY }) => {
  const [typedLengths, setTypedLengths] = useState<number[]>(
    Array(codeLines.length * verticalRepeat).fill(0),
  );
      ...
      timers[i] = window.setTimeout(() => {
        const interval = window.setInterval(() => {
          setTypedLengths((prev) => {
            ...
            if (
              next[i] <
              codeLines[i % codeLines.length].length * repeatCount
            ) {
              next[i]++;
            }
            return next;
  ...
  return (
    <div className="absolute inset-0 w-full h-full z-0 pointer-events-none flex flex-col justify-between overflow-x-hidden">
      {Array.from({ length: codeLines.length * 5 }).map((_, i) => {
        const line = Array(repeatCount)
          .fill(codeLines[i % codeLines.length])
          .join("   ");
        ...
        return (
          <div
            ...
            style={{
              background:
                "linear-gradient(90deg, #00ff99cc 0%, #00cfffcc 100%)",
